...
def get_screenshot(streamlit_path,file_name):
    path=os.path.join(streamlit_path, '%s.py'%file_name)
    proc,final_port=streamlit_service(path)
    server_url = "http://"+get_ip()+":"+str(final_port)
    pic_path=os.path.join(streamlit_path, 'img', '%s.png'%file_name)

    options = Options()
    options.headless = True

    binary = None if Config.FIREFOX_BINARY is None else FirefoxBinary(Config.FIREFOX_BINARY)
    with webdriver.Firefox(options=options,firefox_binary=binary) as browser:
        ...
        try:
            ...
            browser.quit()
    psutil.Popen("taskkill /F /T /PID %i"%proc.pid , shell=True)
    return pic_path
...
def streamlit_service(path,file_name=None,modified=None,running_streamlits=None):
    ...
    if running_streamlits is not None or file_name is not None:
        ...
        if file_name in running_streamlits: #process may have stopped or file modified
            if proc_running(running_streamlits[file_name]["proc"]):
                ...
                psutil.Popen("taskkill /F /T /PID %i"%curproc.pid , shell=True)
    ...
    proc = psutil.Popen(command,shell=True,
             stdin=None, stdout=None, stderr=None, close_fds=True, env=my_env)
