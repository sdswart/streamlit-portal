...
app = Flask(__name__,template_folder='templates',static_url_path=base_path+'/static')
...
def upload():
    if request.method == 'POST':
        ...
        if file:
            ...
            path=os.path.join(Config.STREAMLIT_PATH, file.filename)
            file.save(path)
...
def delete(name):
    path=os.path.join(Config.STREAMLIT_PATH, "%s.py"%name)
    if os.path.isfile(path):
        os.remove(path)
        img_path=os.path.join(Config.STREAMLIT_PATH, 'img', '%s.png'%name)
...
def serve_img(name):
    path=os.path.join(Config.STREAMLIT_PATH, 'img', '%s.png'%name)
    print(path)
...
def create_img(name):
    ...
    if os.path.isfile(path):
        try:
            ...
            img_path=get_screenshot(Config.STREAMLIT_PATH,name)
...
def home():
    ...
    #All Streamlits
    streamlits={}
    for file in os.listdir(Config.STREAMLIT_PATH):
        name=os.path.splitext(file)[0]
        if file.endswith(".py"):
            img_path=path=os.path.join(Config.STREAMLIT_PATH, 'img', '%s.png'%name)
            ...
            streamlits[name]={"img_url":img_url,
                            ...
                            "file_path":os.path.join(Config.STREAMLIT_PATH, '%s.py'%name),
